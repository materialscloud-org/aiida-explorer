...
module.exports = function (grunt) {
	...
	var appConfig = {
		...
		hostAiidaProductionBackend:
			'https://aiida-dev.materialscloud.org',
	...
	grunt.initConfig({
		...
		sass: {
			...
			compile: {
				files: {
					'<%= yeoman.app %>/styles/css/theme.css': [
						'<%= yeoman.app %>/styles/sass/theme.scss',
					],
				},
			},
		},
				...
				options: {
					...
					middleware: function (connect, options) {
						return [
							...
							connect().use('/docs', serveStatic('./docs')),
			...
			development: {
				...
				constants: {
					ENV: {
						name: 'development',
						hostAiidaBackend:
							'<%= yeoman.hostAiidaDevelopmentBackend %>',
						exploreOwnRestEndPoint: 'http://127.0.0.1:5000/api/v4',
						commonRestEndPoint:
							'https://dev-www.materialscloud.org/mcloud/api/v2',
						profilesUrl:
							'https://dev-www.materialscloud.org/mcloud/api/v2/explore/profiles',
						logosUrl:
							'https://dev-www.materialscloud.org/mcloud/api/v2/explore/logos',
					},
				},
			},
			production: {
				...
				constants: {
					ENV: {
						...
						exploreOwnRestEndPoint:
							'https://dev-www.materialscloud.org/api/v2',
						commonRestEndPoint:
							'https://dev-www.materialscloud.org/mcloud/api/v2',
						profilesUrl:
							'https://dev-www.materialscloud.org/mcloud/api/v2/explore/profiles',
			...
			options: {
				...
				patterns: {
					js: [
						[
							/(scripts\/(\w*\/)*[^''""]*\.(js))/g,
							'Replacing references to js files',
						],
						[
							/(styles\/(\w*\/)*[^''""]*\.(css))/g,
							'Replacing references to css files',
						],
					], //Scan js files to update reference
					css: [
						[
							/(images\/(\w*\/)*[^''""]*\.(png|jpg|jpeg|gif|svg))/g,
							'Replacing references to images',
						],
						[
							/(styles\/(\w*\/)*[^''""]*\.(ttf|woff|woff2))/g,
							'Replacing references to fonts',
						],
	...
	grunt.registerTask('build', ['localbuild']);

...
angular.module("materialsCloudApp").controller('ServerconnectCtrl',
    ["$scope", "$state", "messageService", "serverConnectService", "utils", "CONFIG",

        function ($scope, $state, messageService, serverConnectService, utils, CONFIG) {
            ...
            $scope.connectServer = function () {
                ...
                if($scope.connectForm.serverendpoint.$valid) {
                    ...
                    messageService.updateMessage("Invalid URL pattern [should be http://ip_or_hostname/api/etc]!", CONFIG.MESSAGE_TYPE.ERROR);
                }
            };

            $scope.checkServerUp = function (endpoint, redirect) {
                serverConnectService.testServer(endpoint)
                // handle success
                    .then(function (data) {
            ...
            $scope.exploreData = function () {
                $state.go("main.explore.dashboard", {selectedProfile: 'ownrestapi'});
            };
            ...
            $scope.resetServer = function () {
                ...
                messageService.updateMessage("Reset to default REST endpoint!", CONFIG.MESSAGE_TYPE.SUCCESS);
            };

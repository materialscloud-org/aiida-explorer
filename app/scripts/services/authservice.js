...
angular.module("materialsCloudApp")
    .factory("authService",  ["$q", "$http", "$cookies", "$rootScope", "utils", "CONFIG",
        ...
        function ($q, $http, $cookies, $rootScope, utils, CONFIG) {
            ...
            var service = {
                ...
                setUser: function(token) {
                    ...
                    expiryTime.setHours(expiryTime.getHours()+CONFIG.CONSTANT.AUTH.EXPIRY_HOURS);
                    $cookies.put(CONFIG.CONSTANT.AUTH.TOKEN, token, {'expires': expiryTime});
                },
                ...
                signin: function(email, password) {
                    ...
                    $http.post(utils.getRestServerEndpoint() + CONFIG.REST_API.AUTH.SIGNIN, {email: email, password: password})
                    ...
                    // handle success
                        .then(function successCallback(response) {
                                ...
                                $rootScope.$broadcast(CONFIG.EVENT_TYPE.AUTH.SIGNIN_SUCCESS);
                                deferred.resolve(response.data);
                            },

                            // handle error
                            function errorCallback(response) {
                                $rootScope.$broadcast(CONFIG.EVENT_TYPE.AUTH.SIGNIN_FAILURE);
                ...
                signInAsGuest: function() {
                    ...
                    // send a post request to the server
                    $http.get(utils.getRestServerEndpoint() + CONFIG.REST_API.AUTH.SIGNIN)
                ...
                testSignInAsGuest: function(serverEndpoint) {
                    ...
                    // send a post request to the server
                    $http.get(serverEndpoint + CONFIG.REST_API.AUTH.SIGNIN)
                 ...
                 * @returns {object} deferred promise with server's response.
                 */
                signup: function(email, password, first_name, last_name, institute) {

                    // create a new instance of deferred
                    var deferred = $q.defer();

                    // send a post request to the server
                    $http.post(utils.getRestServerEndpoint() + CONFIG.REST_API.AUTH.SIGNUP, {email: email, password: password, first_name: first_name, last_name: last_name, institute: institute})

                    // handle success
                        .then(function successCallback(response) {
                                console.log(response);
                                $rootScope.$broadcast(CONFIG.EVENT_TYPE.AUTH.SIGNUP_SUCCESS);
                                deferred.resolve({user: response.data.response});
                            },

                            // handle error
                            function errorCallback(response) {
                                $rootScope.$broadcast(CONFIG.EVENT_TYPE.AUTH.SIGNUP_FAILURE);
                                deferred.reject(response);
                            });

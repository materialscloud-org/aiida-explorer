		...
		function (
			...
			$stateProvider
				...
				.state('main.explore.help', {
					...
					resolve: {
						$title: function () {
							return 'Add new EXPLORE section';
						},
					},
				})
				...
				.state('main.explore.dashboard', {
					...
					controller: function (
						...
						function gotoPage() {
							...
							$scope.selectedProfileDisplayName =
								$scope.selectedProfileInfo.TITLE;
							$scope.profileRestEndPoint =
								$scope.selectedProfileInfo.RESTENDPOINT;
							$scope.typeColumnName =
								CONFIG.GRID[
									'FILTERS_' +
										$scope.selectedProfileInfo.REST_API_MAJOR_VERSION.toUpperCase()
								]['TYPE_COLUMN_NAME'];

							if (
								currentState === 'main.explore.dashboard.statistics'
							) {
								$scope.selectedView = 'statisticsView';
								statetogo = 'main.explore.dashboard.statistics';
							} else if (
								currentState.indexOf(
									'main.explore.dashboard.details'
								) >= 0
							) {
								$scope.selectedView = 'detailsView';
								if (currentState == 'main.explore.dashboard.details')
									statetogo = 'main.explore.dashboard.details';
								else statetogo = currentState;
							} else if (
								currentState.indexOf('main.explore.dashboard.grid') >=
									0 ||
								currentState === 'main.explore.dashboard'
							) {
								$scope.selectedView = 'gridView';

								if (
									currentState === 'main.explore.dashboard' ||
									currentState == 'main.explore.dashboard.grid'
								)
								...
								nodeService.getServerInfo(endpoint).then(
									function (response) {
										if ('API_major_version' in response.data.data) {
											restApiMajorVersion =
												'v' + response.data.data.API_major_version;
											restApiMinorVersion = parseInt(
												response.data.data.API_minor_version
											);
										} else {
											var restApiVersion = response.data.data[0];
											var tmp = restApiVersion.length;
											restApiMajorVersion = restApiVersion.substring(
												tmp - 2,
												tmp
											);
										}
										$scope.selectedProfileInfo = {
											...
											TITLE:
												'Your AiiDA profile (url:' + endpoint + ')',
						...
						if ($scope.selectedProfile == 'ownrestapi') {
							...
							if (baseUrl) {
								if (utils.isUrlValid(baseUrl)) {
									...
									messageService.updateMessage(
										'base_url (URL) parameter value is not a valid URL format!',
										CONFIG.MESSAGE_TYPE.ERROR
									);
							} else {
								endpoint = utils.getUserServerEndpoint();
								if (endpoint) setApiVersion(endpoint);
								else {
									$state.go('main.serverconnect');
								}
							}
						} else {
							$scope.selectedProfileInfo = utils.getExploreProfileInfo(
					...
					resolve: {
						...
						$title: [
							'$title',
							'getSelectedProfile',
							function ($title, getSelectedProfile) {
								return (
									$title + ' - ' + getSelectedProfile.selectedProfile
								);
							},
						],
					},
				...
				.state('main.explore.dashboard.grid', {
					...
					resolve: {
						...
						loadMyCtrl: [
							'loadMyService',
							'$ocLazyLoad',
							function (loadMyService, $ocLazyLoad) {
								return $ocLazyLoad.load([
									{
				...
				.state('main.explore.dashboard.grid.computers', {
					...
					resolve: {
						...
						loadMyCtrl: [
							...
							function ($ocLazyLoad) {
								return $ocLazyLoad.load([
									'scripts/controllers/explore/computers.js',
								]); // Resolve promise and load before view
							},
						],
					},
				...
				.state('main.explore.dashboard.grid.calculations', {
					...
					resolve: {
						...
						loadMyCtrl: [
							...
							function ($ocLazyLoad) {
								return $ocLazyLoad.load([
									'scripts/controllers/explore/calculations.js',
								]); // Resolve promise and load before view
							},
						],
					},
				...
				.state('main.explore.dashboard.grid.data', {
					...
					resolve: {
						...
						loadMyCtrl: [
							...
							function ($ocLazyLoad) {
								return $ocLazyLoad.load([
									'scripts/controllers/explore/datanodes.js',
								]); // Resolve promise and load before view
							},
						],
					},
				...
				.state('main.explore.dashboard.grid.codes', {
					...
					resolve: {
						...
						loadMyCtrl: [
							...
							function ($ocLazyLoad) {
								return $ocLazyLoad.load([
									'scripts/controllers/explore/codes.js',
								]); // Resolve promise and load before view
							},
						],
					},
				...
				.state('main.explore.dashboard.details', {
					...
					controller: function ($scope, $stateParams, utils, $state) {
						...
						function gotopage() {
							if (
								$state.current.name ===
								'main.explore.dashboard.details.viewnode'
							) {
								$state.go('main.explore.dashboard.details.viewnode');
							}
						}

						if ($scope.selectedProfileInfo) gotopage();
				...
				.state('main.explore.dashboard.details.viewnode', {
					...
					resolve: {
						...
						$title: [
							'getNodeId',
							'getNodeType',
							function (getNodeId, getNodeType) {
								return (
									getNodeId.nodeId + ' (' + getNodeType.nodeType + ')'
								);
							},
						],
					},
				...
				.state('main.explore.dashboard.statistics', {
					...
					resolve: {
						...
						$title: [
							...
							function (getSelectedProfile) {
								return (
									getSelectedProfile.selectedProfile + ' statistics'
								);
							},
						],
						loadMyCtrl: [
...
function loadProfileConfig() {
	...
	return $http.get(profilesUrl).then(
		function (response) {
			var profiles = {};
		...
		function (err) {
			console.error(
				'Error loading the application environment.',
				err
			);
		}
	);
